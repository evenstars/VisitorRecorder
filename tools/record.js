...
function main() {
    let person = readInfo('tools/records/info.txt');
    person.name = `'${person.name}'`;
    let imageNames = getImages('tools/records/images');

    try { 
        faceActions.performRecordVisitings(person, imageNames, result => {
            process.exit(0);
        });
    }
    catch (e) {
...
function getImages(dir) {
    ...
    let renamedList = fileList.map(fileName => {
        ...
        let newPath = `${ftp.paths.recognize}/${newName}`;
